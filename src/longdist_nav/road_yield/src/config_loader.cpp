...
ManagerConfig loadManagerConfig(const std::string& path)
{
  ...
  config.oncoming_confirm_time =
      yamlValue<double>(root, "oncoming_confirm_time", config.oncoming_confirm_time);
  ...
  if (!root["route_waypoint_file"] || !root["avoidance_waypoint_file"] || !root["road_boundary_file"])
  {
    throw std::runtime_error(
        "configuration must specify route_waypoint_file, avoidance_waypoint_file and road_boundary_file");
  }
  const bool csv_yaw_degrees = yamlValue<bool>(root, "csv_yaw_degrees", true);
  const std::string route_path = resolvePath(path, root["route_waypoint_file"].as<std::string>());
  const std::string avoidance_path = resolvePath(path, root["avoidance_waypoint_file"].as<std::string>());
  const std::string boundary_path = resolvePath(path, root["road_boundary_file"].as<std::string>());

  config.route = loadPoseCsv(route_path, "route_", csv_yaw_degrees);
  const std::vector<NavigationPose> shelters = loadPoseCsv(avoidance_path, "avoidance_", csv_yaw_degrees);
  ...
  if (config.control_rate <= 0.0 || config.stair_publish_rate <= 0.0 || config.perception_timeout <= 0.0 ||
      config.avoidance_wait_time < 0.0 || config.ahead_margin < 0.0)
  {
  ...
  ROS_INFO_STREAM("Loaded patrol=" << config.route.size() << ", avoidance="
                  ...
                  << config.road_cross_sections.size() << ", detection patrol indexes="
                  << config.detection_route_begin + 1 << ".." << config.detection_route_end + 1);
  return config;
}
