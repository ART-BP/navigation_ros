...
`/teb_obstacle_sim/markers`.

```bash
roslaunch teb_obstacle_sim simulator.launch
```

The default definitions are in `config/obstacles.yaml`. Each obstacle
...
```bash
roslaunch teb_obstacle_sim simulator.launch use_config:=false
```
