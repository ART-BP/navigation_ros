...
public:
      ...
      : private_nh_("~")
  {
    ...
    private_nh_.param<std::string>("marker_topic", marker_topic_, "/teb_obstacle_adapter/markers");
...
private:
      ...
      costmap_converter::ObstacleMsg& obstacle) const
  {
    ...
    const auto& linear_velocity = tracked_object.twist.twist.linear;
    if (!tracked_object.has_linear_velocity || !isFinite(linear_velocity.x) || !isFinite(linear_velocity.y) ||
        std::hypot(linear_velocity.x, linear_velocity.y) < min_velocity_)
    {
...
{
  ros::init(argc, argv, "teb_obstacle_adapter");
