...
class NeuPANServiceServer:
    def __init__(self):
        ...
        self.initial_path_publish_rate = float(
            rospy.get_param("~initial_path_publish_rate", 1.0)
        )
        self.last_initial_path_publish_time = None
    ...
    def compute_velocity(self, robot_pose=None, use_tf_pose=True):
        ...
        try:
            with self.lock:
                ...
                if self.should_publish_initial_path():
                    self.initial_path_pub.publish(
                        self.path_msg_from_neupan_points(
                            self.neupan_planner.initial_path
                        )
                    )
    ...
    def should_publish_initial_path(self):
        if self.initial_path_publish_rate <= 0.0:
            return False

        now = rospy.Time.now()
        if self.last_initial_path_publish_time is None:
            self.last_initial_path_publish_time = now
            return True

        interval = 1.0 / self.initial_path_publish_rate
        if (now - self.last_initial_path_publish_time).to_sec() >= interval:
            self.last_initial_path_publish_time = now
            return True
