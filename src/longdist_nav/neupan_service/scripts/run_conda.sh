...
PY_SCRIPT="${SCRIPT_DIR}/server.py"

if [[ ! -f "${PY_SCRIPT}" ]]; then
  echo "Cannot find server.py next to ${BASH_SOURCE[0]}" >&2
  exit 1
fi

export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/matplotlib}"
export XDG_CACHE_HOME="${XDG_CACHE_HOME:-/tmp/neupan_cache}"
mkdir -p "${MPLCONFIGDIR}" "${XDG_CACHE_HOME}/fontconfig" 2>/dev/null || true
...
if [[ -n "${NEUPAN_PYTHON:-}" ]]; then
  ...
  CONDA_SETUP="${NEUPAN_CONDA_SETUP:-${HOME}/mydrive/anaconda3/etc/profile.d/conda.sh}"
