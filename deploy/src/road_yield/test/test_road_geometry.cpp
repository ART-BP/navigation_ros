...
TEST(RoadGeometry, EmptyGeometryDisablesDetectionSafely)
{
  const rym::RoadGeometry road({});
  EXPECT_DOUBLE_EQ(0.0, road.length());
  EXPECT_FALSE(road.contains({0.0, 0.0}));
  EXPECT_FALSE(road.isAheadTowardGoal({1.0, 0.0}, {0.0, 0.0}, {2.0, 0.0}, 0.3, 30.0));
}
